...
<h1 align="center">Gald3r for Claude Code</h1>

<p align="center">
  The <strong>Claude Code edition</strong> of Gald3r -- file-based memory, task management, and
  agent orchestration that runs <strong>inside Claude Code</strong>. Part of the Gald3r framework
...
This is the **Claude Code distribution** of Gald3r. The [`claude/`](./claude/) folder is a
complete, ready-to-deploy gald3r setup tuned for Claude Code -- drop its contents into your
...
```bash
git clone https://github.com/wrm3/gald3r_platform_claude.git
cp -r gald3r_platform_claude/claude/. /path/to/your/project/
```

Then open the project in Claude Code and type `@g-status` to confirm gald3r loaded.

> The `claude/` payload is **additive** -- it drops in gald3r's config + `.gald3r/` state and
