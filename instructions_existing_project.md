# Adding Gald3r to an Existing Project (Claude Code)

Add gald3r to a project that **already exists**, for use with **Claude Code**.

For a brand-new project, see [instructions_new_project.md](./instructions_new_project.md).

---

## Step 1 - Get the `claude/` payload

```bash
git clone --depth 1 https://github.com/wrm3/gald3r_platform_claude.git
```
...
**bash / macOS / Linux:**
```bash
cp -r gald3r_platform_claude/claude/. /path/to/your/existing/project/
```

**Windows PowerShell:**
```powershell
Copy-Item -Recurse -Force .\gald3r_platform_claude\claude\* C:\path\to\your\project\
```
...
Reload the project in Claude Code. gald3r auto-loads from its config.
